    ...
    try {
      ...
      if (page >= totalPages) {
        return interaction.editReply(
          `⚠️ Chỉ có ${totalPages} trang lịch sử.`
        );
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setFooter({ text: `Trang ${page + 1}/${totalPages} | Tổng: ${history.length} bài` });

      await interaction.editReply({ embeds: [embed] });
    } catch (error) {
      console.error("Lỗi trong lệnh history:", error);
      return interaction.editReply("❌ Đã xảy ra lỗi khi hiển thị lịch sử!");
    }
  },
};
